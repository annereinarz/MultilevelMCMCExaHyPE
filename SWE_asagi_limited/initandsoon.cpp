...
//    tarch::logging::CommandLineLogger::getInstance().clearFilterList();
  ...
  std::vector<double> run_exahype(std::vector<double> param_, int subcommunicator_rank, int level=0){
    muq::param.resize(2);
    int numProbes = 4;
    muq::solution.resize(numProbes);
    param=param_;  //store parameters

    //check if all levels have solvers
    int numLevelsExaHyPE = exahype::solvers::RegisteredSolvers.size();
    assert(numLevelsExaHyPE > level);
    /*if(!firstRun){
